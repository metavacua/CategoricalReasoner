...
Catty is not under any institution, is not homework or any such instrument, is not subject to a thesis defense committee, or similar organizational commitments; it is a thesis in the sense of being held to formal standards for dissertation work at the level of the formation and operation of the instrument itself with voluntarily adopted academic standards for citations and scholarship. It is under the auspice of the Invisible College; the Invisible College is contemporaneously a sociological concept related to the informal conduct of scholarship outside of established institutions, and it has direct roots into the common practice of science in the 19th and 20th century; it is in the tradition of Rutherford and Tesla.
...
The `docs/dissertation/` directory contains LaTeX documentation that documents the research and development process. These files explain theoretical foundations, design decisions, and algorithmic approaches. They are supporting documentation for the algorithmic contributions, not the primary deliverables themselves.

**Note**: The `thesis/` directory referenced in older documentation is deprecated. Use `docs/dissertation/` instead.

## Project Structure

```
├── docs/dissertation/           # LaTeX thesis documentation
...
- Semantic web processing and reasoning (Apache Jena)
- Code generation for validation and transformation
- Unit testing and validation frameworks (JUnit)
...
**Prohibited**: Creating local ontology files (JSON-LD, TTL, OWL) that define schemas. All semantic web data must be consumed from external sources.

See `docs/external-ontologies.md` for information about consumed data sources and integration approaches.

## Citation Management

The citation registry is located at `docs/dissertation/bibliography/citations.yaml`. 

- Use ONLY citation keys from this registry
- Adding new verified citations is encouraged
- All citation additions must be verified via Crossref, Wikidata, or other authoritative sources
