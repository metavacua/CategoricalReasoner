...
\begin{itemize}
  ...
  \item Updated \texttt{.github/workflows/deploy.yml} to build from \texttt{docs/dissertation}, run build preview and build-and-deploy on pull requests, deploy on non-PR events, emit site output to a consistent directory, and publish a bookshelf HTML stack for dissertation and structural rules.
...
\begin{itemize}
  \item Workflow validation: \textbf{failed locally} due to missing \texttt{pyyaml} dependency.
    \begin{itemize}
      \item Command: \texttt{python src/scripts/validate\_workflows.py}
      \item Error: \texttt{ModuleNotFoundError: No module named 'yaml'}
    \end{itemize}
