...
\begin{itemize}
  ...
  \item The refinement adds preamble-only command checks to the validator to approximate compilation semantics for included files.
\end{itemize}
...
\begin{enumerate}
  ...
  \item Extend the TeX validator with preamble-only command checks to narrow the validator/compiler gap.
...
\begin{itemize}
  ...
  \item Updated \texttt{src/benchmarks/run.py}:
    ...
    \end{itemize}
  \item Extended the TeX structure validator with preamble-only command checks.
