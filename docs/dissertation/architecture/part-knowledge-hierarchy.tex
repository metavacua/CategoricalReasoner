...
Knowledge is consumed and validated according to the following priority chain:
\begin{enumerate}
    \item \textbf{Wikidata (QIDs)}: Primary source for universal concepts.
    \item \textbf{DBPedia}: Secondary source for broad linked-data entities.
    \item \textbf{Formal Standards}: ISO/IEC, IETF, and academic specifications.
    \item \textbf{Original Research}: Specific innovations of the Catty thesis.
\end{enumerate}
...
If a QID is suspected, it must be verified via SPARQL:
...
If a QID is unknown, it must be discovered using the Wikidata Search API or a label-based SPARQL query:
\begin{lstlisting}[language=SQL]
