...
\subsection*{Queries Requiring External Standard Integration}

The 13 SPARQL queries in \code{docs/sparql-examples.md} reference vocabulary that must be \textbf{replaced with existing W3C/OMG/ISO standards} rather than authored locally. These queries are classified as \textbf{PENDING REFACTORING} to consume from:

\begin{itemize}
    \item \textbf{Logic \& Axioms}: W3C OWL 2 (\code{owl:Ontology}, \code{owl:Axiom}) instead of \code{catty:Logic}
    \item \textbf{Category Theory}: ISO Common Logic/COLORE (\code{theory:domain}, \code{theory:morphism}) instead of \code{catty:Extension}
    \item \textbf{Logic Translations}: OMG DOL (\code{dol:LogicTranslation}) instead of \code{catty:CurryHowardFunctor}
    \item \textbf{Functors}: COLORE Category Theory module instead of \code{catty:AdjointFunctors}
\end{itemize}

These queries will be refactored to import and consume from external standards per AGENTS.md constraints on code reuse.
