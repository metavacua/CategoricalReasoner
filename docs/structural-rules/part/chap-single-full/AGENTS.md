# AGENTS.md - chap-single-full: Single Antecedent, Full Context RHS

## Scope
This chapter investigates weakening under the **Single/Full** context configuration: the left-hand side (antecedent) is restricted to at most one formula, while the right-hand side (succedent) can be freely extended.

This is a **context configuration**, not a named logic. Dual intuitionistic logic is an example of a logic that satisfies this configuration, but the chapter defines the structural rule behavior, not a specific logic.
...
| Single | Full | `chap-single-full` (this) |
...
## Special Consideration: Single Antecedent
The restriction to at most one formula on the LHS relates to:
- The role of ⊤ (top/verum)
- Dual logic constructions
- Minimal logic dual to standard intuitionistic logic

## Theorem Scoping
Theorems in this chapter:
- Apply only within this chapter (`chap-single-full`)
