# AGENTS.md - Structural Rules: Part II (Contraction)
...
```
FUNCTION AssembleContractionPart(output_file):
    # Initialize the main document
    INITIALIZE latex_document with preamble
    
    # Add Part declaration
    ADD "\\part{Contraction}" TO latex_document
    ...
    FOR each chapter_dir IN ["chap-symmetric-full-context-lhs-rhs",
        ...
        FOR each section_dir IN ["sec-full-contraction",
...
1. **Part-level files**: `part-contraction.tex` provides structure and `\input{}` directives
...
- `\part{Contraction}` - Part declaration
...
This part covers:
1. **Symmetric Full Context (Classical LK)**: Both LHS and RHS have contraction
2. **Asymmetric Full Context LHS / Single Succedent (Intuitionistic LJ)**: LHS contraction, RHS single formula
3. **Asymmetric Single Antecedent / Full Context RHS (Dual Intuitionistic LDJ)**: LHS single formula, RHS contraction
4. **Symmetric Single Antecedent and Single Succedent (Minimal Logic)**: Both restricted

Each chapter contains four sections:
- **Full Contraction**: Classical contraction
- **Linear Contraction**: Resource-sensitive via exponential modality
- **Affine Contraction**: Contraction rejected, weakening admitted
- **Relevant Contraction**: Contraction in relevance logic
