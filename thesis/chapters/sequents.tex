...
The reflexive axiom $\frac{\Gamma \vdash A}{\Gamma \vdash A}$ forms the foundation of sequent calculus presentations. In LK, this represents the basic identity principle. Different substructural calculi restrict or modify this axiom scheme, producing characteristically different logical behaviors.

The matrix of reflexive axiom schemata determines what structures and operations a logic can prove. If a logic proves every reflexive axiom scheme of a given form, it will have specific structures or operations and a characterizing cut metarule. If a logic does not have the corresponding structures and operations, then either they're implicit or the logic is consistently characterized with respect to the reflexive axiom scheme.
...
\begin{itemize}
    \item Left contraction: $\frac{\Gamma, A, A \vdash \Delta}{\Gamma \vdash \Delta}$
...
The matrix or vectorization of logical relations between units, axioms, rules, and operations is a key result that allows the automatic construction of various categorizable logics in relation to each other.
...
This functional incompleteness is not a deficiency but a feature that enables the logical pluralism Catty advocates. The relationship between material implication and minimal logic is explored in detail by Diener and McKubre-Jordens \cite{diener2016classifying}, who classify various forms of material implication over minimal logic.

\section*{From LK to Substructural Calculi: The Restriction Matrix}
