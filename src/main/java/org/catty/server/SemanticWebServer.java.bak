...
public class SemanticWebServer {
    ...
    private void loadCattyOntologies() throws IOException {
        ...
        if (files != null) {
            ...
            try {
                ...
                if (dataset.isInTransaction() && ((DatasetGraphTransaction)dataset.asDatasetGraph()).isInTransaction()) {
                    dataset.end();
                }
            }
        }
    ...
    /**
     * Starts the semantic web server.
     * 
     * @throws IOException if the server cannot be started
     */
    public void start() throws IOException {
        LOG.info("Starting Semantic Web Server on {}:{}{}", host, port, datasetName);
        ...
        try {
            ...
            server = FusekiServer.create()
                    ...
                    .loopback(true) // Localhost-only binding
                    .port(port)
                    .build();
            
            server.start();
            
            LOG.info("Semantic Web Server started successfully");
            LOG.info("SPARQL Query Endpoint: http://{}:{}{}/query", host, port, datasetName);
            LOG.info("SPARQL Update Endpoint: http://{}:{}{}/update", host, port, datasetName);
            LOG.info("Fuseki Service UI: http://{}:{}{}", host, port, datasetName);
    ...
    /**
     * Stops the semantic web server.
     */
    public void stop() {
        LOG.info("Stopping Semantic Web Server");
     ...
     * 
     * @return the FusekiServer instance, or null if not started
     */
    ...
    /**
     * Main entry point for the Semantic Web Server.
     * 
     * @param args command line arguments (optional: [host] [port] [dataset])
     */
    public static void main(String[] args) {
        ...
        if (args.length >= 3) datasetName = args[2];
        ...
        try {
            SemanticWebServer server = new SemanticWebServer(host, port, datasetName);
            server.start();
            ...
            LOG.error("Failed to start Semantic Web Server", e);
            System.exit(1);
